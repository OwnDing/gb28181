...
- 在线预览：
  ...
  - `POST /api/preview/stop`
  - `GET /api/preview/status`
  - 已接通 ZLMediaKit `openRtpServer/closeRtpServer`
  - 已接通 GB28181 `INVITE/BYE`
  - 已支持 H.264 / H.265 设备编码字段与浏览器能力判断
  - 同通道会话复用（viewerCount）
- 存储策略：
  - `GET /api/storage/policy`
  - `PUT /api/storage/policy`
...
- GB28181 扩展能力：
  - 查询下发：
    - `POST /api/gb28181/devices/{deviceId}/queries/device-info`
    - `POST /api/gb28181/devices/{deviceId}/queries/catalog`
    - `POST /api/gb28181/devices/{deviceId}/queries/records`
  - 查询结果读取：
    - `GET /api/gb28181/devices/{deviceId}/profile`
    - `GET /api/gb28181/devices/{deviceId}/catalog`
    - `GET /api/gb28181/devices/{deviceId}/records`
    - `GET /api/gb28181/alarms`
    - `GET /api/gb28181/mobile-positions`
  - 订阅管理：
    - `POST /api/gb28181/devices/{deviceId}/subscriptions`
    - `DELETE /api/gb28181/subscriptions/{id}`
    - `GET /api/gb28181/subscriptions`
  - 回放会话查询：
    - `GET /api/gb28181/playback-sessions`
  - SIP 侧已支持：
    - 设备 `REGISTER/Keepalive` 在线状态维护
    - 平台 `MESSAGE` 查询发送
    - 平台 `SUBSCRIBE` 订阅发送
    - 设备 `MESSAGE` 上报解析并落库：`DeviceInfo/Catalog/RecordInfo/Alarm/MobilePosition`
- 数据层：
  - SQLite 初始化脚本与表结构已落地
  - 扩展表已落地：`gb_device_profile`、`gb_record_item`、`gb_alarm_event`、`gb_mobile_position`、`gb_subscription`、`gb_playback_session`
...
- 在线预览页：`/video-preview`
  - 支持多路并发预览（多会话）
  - 支持单路停止、全部停止、会话状态刷新
- 存储设置页：`/storage-settings`
- GB28181 能力中心：`/gb28181`
  - 设备信息查询、目录查询、录像查询
  - 订阅创建/取消
  - 目录、录像、报警、移动位置、回放会话结果展示

### 15.3 当前构建与测试状态
- 后端测试：`.\mvnw.cmd -q test` 通过
- 前端构建：`npm run build` 通过

## 16. 待实现功能（下一阶段）

### 16.1 协议完整性（重点）
- 完整对齐 GB28181-2011/2016/2022 全量能力仍未完成
- 回放控制链路待完善：
  - 回放 `INVITE` 发起与独立会话管理
  - `INFO` 控制（暂停、恢复、倍速、拖动定位）
- 云台 PTZ 控制待实现（前后端 + SIP 控制报文）
- 查询/订阅结果的分页、分批、SN 关联与超时重试策略待完善
- 厂商差异兼容适配（不同设备 XML 字段与行为差异）待完善

### 16.2 媒体与兼容性
- H.265 转码兜底链路（FFmpeg 可选）尚未实现
- 预览失败自动回退策略（WebRTC -> HLS/HTTP-FLV 的自动重试）可继续增强

### 16.3 工程化与运维
- 端到端联调测试（真实摄像头/NVR + ZLM）脚本化验证待完善
- 运行监控指标（SIP 成功率、预览成功率、存储压力）待完善
- 部署文档与生产参数模板（端口、防火墙、ZLM 配置）待补充
